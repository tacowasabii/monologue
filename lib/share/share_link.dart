...
const shareHost = 'monologue.ink';

final shareApiBase = Uri.https(shareHost, '/api/shares');
...
/// 메신저에서 누르는 `https://monologue.ink/s/<ID>`와
/// 웹 보기의 "앱에서 열기"가 여는 `monologue://s/<ID>`를 받는다.
String? shareIdFromUri(Uri uri) {
  final segments = uri.pathSegments.where((s) => s.isNotEmpty).toList();
  String? candidate;
  if (uri.scheme == 'https' && uri.host == shareHost && segments.length == 2 && segments[0] == 's') {
