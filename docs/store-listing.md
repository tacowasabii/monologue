...
Android 앱에 들어 있는 Google ML Kit(글자 인식)가 진단 정보를 Google로 보내므로 **"수집함"** 으로 답한다. 사용자가 링크로 공유한 대본(작품명·본문·성별·나이대·태그·고른 경우 노트)도 서버에 7일 동안 저장되므로 함께 신고한다. 사진·녹음·메모는 기기 밖으로 나가지 않는다. 근거: [ML Kit Android 데이터 공개](https://developers.google.com/ml-kit/android-data-disclosure) — 기기 정보, 앱 버전, 설치별 식별자, 성능 지표, 이벤트·오류 코드를 HTTPS로 보내고 제3자에게 넘기지 않으며, 끄는 방법은 안내돼 있지 않다.
